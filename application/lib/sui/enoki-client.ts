...
const apiKey = process.env.ENOKI_SECRET_KEY as string;

export const enokiClient: EnokiClient | null = apiKey
  ? new EnokiClient({ apiKey })
  : null;
...
export async function executeAsAdmin(
  ...
  const { signature } = await admin.signTransaction(
