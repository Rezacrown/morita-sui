...
export default function MarketplacePage() {
  ...
  const { data: barterListings = { listings: [], total: 0 } } = useQuery({
    ...
    queryFn: () =>
      getMarketplaceListings({ rarity: rarityFilter ?? undefined }),
  });
  ...
  const filteredSales = saleListings.filter((l) => {
    ...
    if (
      searchValue &&
      !l.itemName.toLowerCase().includes(searchValue.toLowerCase())
    )
      return false;
    return l.isActive;
  });
  ...
  const hasAny =
    (showSales ? filteredSales.length > 0 : false) ||
    (showBarters ? barterListings.listings.length > 0 : false);
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-6">
        {(["all", "sale", "barter"] as Tab[]).map((tab) => (
          <button
            ...
            className={`px-5 py-2.5 font-display font-black text-xs uppercase rounded-xl border-3 border-[#1E2044] transition-all cursor-pointer ${activeTab === tab ? "bg-blueberry text-white shadow-[3px_3px_0px_0px_var(--color-border-dark)]" : "bg-white text-[#1E2044] hover:bg-blueberry-cream"}`}
          >
            {tab === "sale"
              ? "For Sale"
              : tab === "barter"
      ...
      {!hasAny ? (
        ...
        <div className="space-y-8">
          {showSales && filteredSales.length > 0 && (
            <div>
              {activeTab === "all" && (
                <h2 className="font-display font-black text-sm uppercase tracking-tight text-[#1E2044]/60 mb-4">
                  For Sale
                </h2>
              )}
              <div className="space-y-3">
                {filteredSales.map((l) => (
                  <ListingRow
                    ...
                    sellerName={l.sellerAddress.slice(0, 8)}
                    onClick={() => {
                      setBuyListing(l);
                      setBuyItemId(l.itemObjectId);
                    }}
                    disabled={!isLoggedIn}
                  />
                ))}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                {barterListings.listings.map((l) => (
                  <EscrowCard
                    key={l.escrowId}
                    offeredItem={l.offeredItem}
                    initiatorName={l.initiatorAddress?.slice(0, 8) ?? ""}
                    conditions={{
                      itemTypeAccept:
                        l.conditionsJson?.item_type_accept ?? null,
                      rarityAccept: l.conditionsJson?.rarity_accept ?? null,
                    }}
                    isActive={l.isActive}
                    disabled={!isLoggedIn}
                    onClick={() => router.push(`/barter/${l.escrowId}`)}
                  />
                ))}
              </div>
            </div>
      ...
      <ConfirmModal
        isOpen={!!buyItemId}
        title="Buy Item"
        message={`Buy ${buyListing?.itemName ?? ""} for ${buyListing?.price ?? ""} MIST?`}
        confirmLabel={txState === "submitting" ? "Buying..." : "Confirm Buy"}
