...
const sidebarItems = [
  ...
  { label: "Games", href: "/dashboard/games", icon: <Swords className="w-4 h-4" /> },
  { label: "Analytics", href: "/dashboard/analytics", icon: <BarChart3 className="w-4 h-4" /> },
  { label: "Activity", href: "/dashboard/activity", icon: <Activity className="w-4 h-4" /> },
    ...
    return (
      <div className="min-h-screen bg-brand-bg flex items-center justify-center p-6">
        <div className="bg-white border-3 border-[#1E2044] rounded-2xl shadow-[4px_4px_0px_0px_var(--color-border-dark)] p-8 text-center max-w-md">
          ...
          <p className="font-sans text-sm text-[#1E2044]/60 mb-6">Sign in to access your developer workspace and manage games.</p>
          <button onClick={() => setShowLogin(true)} className="px-6 py-3 bg-blueberry text-white border-3 border-[#1E2044] font-display font-black rounded-xl shadow-[3px_3px_0px_0px_var(--color-border-dark)] hover:-translate-y-0.5 transition-all uppercase text-sm cursor-pointer">Sign In</button>
        </div>
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        <header className="sticky top-0 z-50 bg-brand-bg/90 backdrop-blur-md border-b-3 border-[#1E2044] px-4 sm:px-6 py-3">
          <div className="flex items-center justify-between gap-4">
            <h2 className="font-display font-black text-lg uppercase tracking-tight text-[#1E2044]">Dashboard</h2>
            <div className="flex items-center gap-4">
              ...
              <button onClick={handleLogout} className="text-[10px] font-mono font-extrabold uppercase tracking-wider text-[#1E2044]/60 hover:text-red-500 transition-colors cursor-pointer">
                Logout
              </button>
            </div>
          </div>
